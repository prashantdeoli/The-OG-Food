...
class DehradunDishIntelAgent:
    def __init__(
        ...
        self.ua_provider: UserAgent | None = None
        try:
        ...
        for attempt in range(1, self.retries + 1):
            try:
                ...
                if attempt == self.retries:
                    if "ERR_HTTP2_PROTOCOL_ERROR" in msg and not self._proxy_pool():
                        raise RuntimeError(
                            "Blocked by upstream anti-bot (ERR_HTTP2_PROTOCOL_ERROR). Set PROXY_URL to a residential proxy and retry."
                        ) from exc
                    raise
    ...
    async def discover_restaurants(self, page: Page, area: str) -> list[Restaurant]:
        ...
        try:
            ...
            print(f"[WARN] Could not open discovery page for {area}: {exc}")
            return []
        ...
        async with async_playwright() as p:
            for area in areas:
                ...
                try:
                    ...
                    if not restaurants:
                        print(f"[WARN] No restaurants discovered for area: {area}")
                        continue
...
def _runtime_checks() -> None:
    if not os.getenv("PROXY_URL"):
        print("[INFO] PROXY_URL is not set. Zomato may block datacenter/non-residential traffic.")
